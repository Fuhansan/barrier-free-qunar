 ...
 * 专门处理聊天相关的广播消息
 */
public class ChatBroadcastReceiver extends BaseBroadcastReceiver {
    private static final String TAG = "ChatBroadcastReceiver";
    
    private final ChatBroadcastListener listener;
    
    public ChatBroadcastReceiver(ChatBroadcastListener listener) {
...
//               (inputData.userInput != null ? inputData.userInput.length() : 0));
    ...
    protected void onError(Context context, Intent intent, Exception error) {
        Log.e(TAG, "处理广播时发生错误: " + (intent != null ? intent.getAction() : "unknown"), error);
